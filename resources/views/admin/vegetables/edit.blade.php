    ...
    <div class="flex pt-16 overflow-hidden bg-gray-50 dark:bg-gray-900" style="padding-top: 90px">
        <div id="main-content" class="relative w-full h-full overflow-y-auto bg-gray-50 lg:ml-64 dark:bg-gray-900">
            <main>
                <div
                    ...
                    <div class="w-full mb-1">
                        <div class="mb-4">
                            ...
                            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Update product
                            </h1>
                            </h1>
                        </div>
                ...
                <div id="aa">
                    <form action="{{ url('products/' . $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg overflow-hidden shadow-md px-8 pt-6 pb-8 mb-4 ml-10 mr-10">
                        @method('PUT')
                        @csrf
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                                <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Name</label>
                                <input type="text" id="name" name="name" placeholder="{{ $product->name }}" value="{{ old('name', $product->name) }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                            <div class="w-1/2 ml-2">
                                <label for="small_description" class="block text-gray-700 text-sm font-bold mb-2">Small Description</label>
                                <input type="text" id="small_description" name="small_description" value="{{ old('name', $product->description) }}" placeholder="{{ $product->small_description }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                        </div>
                        <div class="mb-4">
                            <label for="description" class="block text-gray-700 text-sm font-bold mb-2">Description</label>
                            <textarea id="description" name="description" rows="3" value="{{ old('name', $product->small_description) }}" placeholder="{{ $product->description }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50"></textarea>
                        </div>
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                                <label for="original_price" class="block text-gray-700 text-sm font-bold mb-2">Original Price</label>
                                <input type="number" id="original_price" name="original_price" value="{{ old('name', $product->original_price) }}" placeholder="{{ $product->original_price }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                            <div class="w-1/2 ml-2">
                                <label for="selling_price" class="block text-gray-700 text-sm font-bold mb-2">Selling Price</label>
                                <input type="number" id="selling_price" name="selling_price" value="{{ old('name', $product->selling_price) }}" placeholder="{{ $product->selling_price }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                        </div>
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                                <label for="image" class="block text-gray-700 text-sm font-bold mb-2">Image</label>
                                <img src="{{ asset('uploads/product/'.$product->image) }}" alt="image here" style="width: 80px">
                        ...
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                                ...
                                <input type="number" id="qty_id" name="qty_id" value="{{ old('name', $product->qty) }}" placeholder="{{ $product->qty }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                            <div class="w-1/2 ml-2">
                                <label for="tax" class="block text-gray-700 text-sm font-bold mb-2">Tax</label>
                                <input type="number" id="tax" name="tax" value="{{ old('name', $product->tax) }}" placeholder="Enter tax" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                        </div>
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                        ...
                        <div class="mb-4 flex">
                            <div class="w-1/2 mr-2">
                                ...
                                <input type="text" id="meta_title" name="meta_title" value="{{ old('name', $product->meta_title) }}" placeholder="Enter meta title" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                            </div>
                            <div class="w-1/2 ml-2">
                                <label for="meta_description" class="block text-gray-700 text-sm font-bold mb-2">meta_description</label>
                                <textarea id="meta_description" name="meta_description" rows="2" value="{{ old('name', $product->meta_description) }}" placeholder="Enter meta description" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50"></textarea>
                            </div>
                        </div>
                        <div class="mb-4">
                            <label for="meta_keywords" class="block text-gray-700 text-sm font-bold mb-2">Meta Keywords</label>
                            <input type="text" id="meta_keywords" name="meta_keywords" value="{{ old('name', $product->meta_keywords) }}" placeholder="Enter meta keywords" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                        </div>
